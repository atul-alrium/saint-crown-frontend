...
const PortfolioBalance = () => {
  ...
  const afd1USDValue =
    afd1Balance && tokenPrice
      ...
      : null;
  setWalletBalanceFormatted(Number(afd1Balance).toFixed(2));
  useEffect(() => {
    ...
    <div className="main-grid">
      ...
      <div className="balance-section">
        <div className="balance-header">
          ...
          <button className="refresh-btn" onClick={loadAccountData}>
            Refresh
          </button>
        </div>
        ...
        <div className="token-balances">
          <div className="token-card">
            ...
            <div className="token-balance" id="afd1Balance">
              {walletBalanceFormatted}
            </div>
            <div className="token-value" id="afd1ValueUSD">
              {afd1USDValue ? `$${afd1USDValue}` : "$0.00"}
            </div>
          </div>
          <div className="token-card usd-card">
            <div className="token-symbol">Account Balance</div>
            <div className="token-balance" id="usdBalance">
              {afd1USDValue}
            </div>
        ...
        <div className="price-display">
          ...
          <div className="current-price" id="currentPrice">
            {error ? "Error" : tokenPrice ? `$${tokenPrice}` : "Loading..."}
          </div>
          <span className="price-change" id="priceChange">
            {priceChange}
          </span>
          <div className="gold-basis" id="goldBasis">
            {" "}
            {error
              ? error
              : goldPrice
